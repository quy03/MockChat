...
class _MessageDetailState extends State<MessageDetail> {
  ...
  // Hàm để gửi tin nhắn lên Firebase
  Future<void> sendMessage() async {
    if (typingController.text.isEmpty) return;

    final DatabaseReference ref = FirebaseDatabase.instance
        .ref(
            "messages/${widget.friendName}") // sử dụng friendName làm ID cuộc trò chuyện
        .push(); // tạo một messageId mới

    await ref.set({
      "senderId": "currentUserId", // Thay bằng ID người dùng hiện tại
      "content": typingController.text,
      "timestamp": DateTime.now().toIso8601String(),
    });

    // Xóa nội dung TextField sau khi gửi
  ...
  Widget build(BuildContext context) {
    ...
    return ChangeNotifierProvider(
      ...
      child: Consumer<ColorProvider>(
        builder: (BuildContext context, ColorProvider iconColoProvider,
            Widget? child) {
          return Scaffold(
            body: SafeArea(
              child: Column(
                                ...
                                Expanded(
                                  child: SingleChildScrollView(
                                    child: BodyMessageDetails(
                                      ...
                                      size: size,
                                      // Sử dụng friendName hoặc conversationId thực tế
                                      conversationId:
                                          'id_of_conversation', // ID của cuộc trò chuyện
                                      friendName: widget.friendName,
                                    ),
                                  ),
                                ),
